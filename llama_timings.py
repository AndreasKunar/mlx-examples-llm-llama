...
def generate(args):
    # #AK# no pause - input("Press enter to start generation")
    print("------")
    print(args.prompt)
    ...
    for token in model.generate(x, args.temp):
        ...
        if len(tokens) == 1:
            ...
            # #AK# add nomber of prompt tokens and token/s
            prompt_processing += " ({} tokens, ".format(len(x[0]))
            prompt_processing += "{:.1f} tokens/s)".format(len(x[0])/(time.time() - start))
    ...
    # #AK# add number of generated tokens and tokens/s
    full_gen += " ({} tokens, ".format(len(tokens)-1) #exclude BOS token
    full_gen += "{:.1f} tokens/s)".format(len(tokens)/(time.time() - start))
    s = tokenizer.decode([t.item() for t in tokens])
    print(s[skip:], flush=True)
    print("------")
    print(prompt_processing)
    print(full_gen)
    print("Note: prompt processing time includes tokenization and model inference time for 1st token.")
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Load time: {(time_load):>8.3f} s")
