...
# performance measuring script for Mixtral
...
def generate(prompt: mx.array, model: Mixtral, temp: Optional[float] = 0.0):
    ...
    y = sample(logits[:, -1, :])
    stats.end_prompt = time.time()        # prompt processing ends with 1st token generation
    yield y
...
if __name__ == "__main__":
    ...
    tokens = []
    for token, i in zip(generate(prompt, model, args.temp), range(args.max_tokens)):
        tokens.append(token)
        if (len(tokens) % 10) == 0:
            mx.eval(tokens)
            eos_index = next(
    ...
    mx.eval(tokens)
    s = tokenizer.decode([t.item() for t in tokens])
    time_prompt = stats.end_prompt - start_gen
    time_response = time.time() - stats.end_prompt
    ...
    time_prompt -= time_1_resp_token
    time_response += time_1_resp_token

    print("[INFO] Statistics:")
    print(f"       load time: {(time_load):>8.3f} s")
    print(f"prompt eval time: {(time_prompt):>8.3f} s / {num_tokens_prompt:5} tokens ",\
          f"({num_tokens_prompt / (time_prompt):>7.2f} token/s)")
    print(f"       eval time: {(time_response):>8.3f} s / {num_tokens_response:5} tokens ",\
          f"({num_tokens_response / (time_response):>7.2f} token/s)")
